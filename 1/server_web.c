...
#define BUFFSIZE	256
#define SERVER_NAME	"Mini web server  V11.1, Paul Girard, Ph.D."
...
#define TIME_PAGE	"<head></head><body><html><h1>The actual date-time in Quebec is : \
<br><br>%s</h1></html></body>\n"

int	recvln(int, char *, int);    	/* read a client http line  */
void send_header(int, int, int, char);  	/* send a http reply header to client*/
...
char * getTxt(char *fname) {
	...
	if (myfile == NULL){
		...
		fileLens = ftell(myfile);
		str = (char *)malloc(fileLens);
		str[0] = '\0';
		rewind(myfile);
		while ((fgets(f, 1000, myfile)) != NULL)
			strcat(str, f);
		fclose(myfile);
		return str;
	}
...
char *getImage(char *path, int *fileLen) {
	FILE *f = fopen(path + 1, "rb");
	if (f == NULL)
		return NULL;
	*fileLen = getFileLen(f);
	char *buf = (char *) malloc(*fileLen), *buf2 = buf;
	while (feof(f) == 0) {
...
int findDot(char *path) {
	int dot;
	for (dot = strlen(path) - 1; path[dot] != '/' && path[dot] != '.'; dot--)
		if (path[dot] == '.')
			break;
	return dot;
}

char *createNewPath(char *path, char *type) {
	int dot = findDot(path);
	if (path[dot] == '.') {
		char *suffix = path + dot + 1;
		path[dot] = '\0';
		if (strstr("HTML HTM html htm", suffix) != NULL) {
			strcat(path, ".html");
			*type = 'h';
		} else if (strstr("JPEG JPG jpg jpeg", suffix) != NULL) {
			strcat(path, ".jpg");
			*type = 'j';
		} else if (strstr("GIF gif", suffix) != NULL) {
			strcat(path, ".gif");
			*type = 'g';
		} else if (strstr("ICO ico", suffix) != NULL) {
			strcat(path, ".ico");
			*type = 'i';
		}else{
			*type = 'o';
...
/* 		Reads & validates the request client */
		...
		printf("Client connection\nRequest type: %s    path : %s     version : %s\n", cmd, path, vers);
...
/* 		Validates the http client request  */
		...
		{		
			send_header(conn, 400, strlen(ERROR_400),'h');
...
*		the message "not found" error */
		...
		{
			send_header(conn, 200, strlen(HOME_PAGE),'h');
			...
			/* testtime.c: test of time function*/
					...
					send_header(conn, 200, strlen(buff),'h');
		       ...
		       { 		/* Error not found */
                   ...
                   char * newPath = createNewPath(path,&type);
				   char * file;
				   int size;
				   ...
				   if(file == NULL){
					   send_header(conn, 404, strlen(ERROR_404),'h');
					   send(conn, ERROR_404, strlen(ERROR_404),0);
					   printf("filename:%s\nError opening file %s\nEnd of client connection\n\n",path,path);
				   }else{
					   send_header(conn, 200, size,type);
					   send(conn, file, size,0);
					   free(file);
				   }
		       }
...
void send_header(int conn, int stat, int len, char type)
{
	char	*statstr, buff[BUFFSIZE] ,*buff2;
...
//	sprintf(buff, "Content-Type: text/html\r\n");
	...
	{
		case 'h':
			buff2 = "Content-Type: text/html\r\n";
			break;
		case 'i':
			buff2 = "Content-Type: image/ico\r\n";
			break;
		case 'j':
			buff2 = "Content-Type: image/jpg\r\n";
			break;
		case 'g':
			buff2 = "Content-Type: image/gif\r\n";
			break;
		default:
			buff2 = "Content-Type: text/html\r\n";
			break;
